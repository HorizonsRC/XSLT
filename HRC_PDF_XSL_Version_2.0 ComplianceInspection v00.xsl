    ...
    <xsl:if test="count($listRows) > 0">
      <xsl:for-each select="$listRows">
        <xsl:choose>
		  ...
		  <!-- ===================================================================================================================================================== -->
          <xsl:when test="$headerOnly = 1 and @Type = 'displaytext' and @Style = 'Panel' and not(name() = 'InspectionType' or (name() = 'ServiceDetails' and $timeValue = '') or (name() = 'Documents' and $imageCount = 0))">
			<xsl:choose>
			<xsl:when test="not(contains('WeatherAssessment,ConditionAssessment',name()))">
			  <p/>
			</xsl:when>
			</xsl:choose>
            <table style="page-break-inside:avoid; table-layout:fixed; margin-left:{$tableIndent}px; width:{$tableWidth}px; font-family:'Calibri'; border-collapse: collapse;">
              ...
              <!-- ================================================================================================================================ -->
			  <xsl:choose>
			  <xsl:when test="not(contains('ErosionSedimentControlGrading,MobileInspection',name()))">
                <tr bgcolor="white" height="10px" style="page-break-inside:avoid;">
          ...
          <!-- ================================================================================================================================ -->
          <xsl:when test="$headerOnly = 0 and name()='WeatherAssessment'">
    ...
    <xsl:choose>
	  <xsl:when test="@Name = 'CONDITION'">
		<xsl:for-each select="./*">
		  ...
		  <xsl:value-of select="//FormData/ConditionRptr/*[$index]/Heading"/>
		  <xsl:choose>
		    <xsl:when test="@Type='option'">
			  <xsl:value-of select="@OptionText"/>
			</xsl:when>			
		    <xsl:when test="@Type='string' and @Description='CONDITION'">
			  <xsl:value-of select="text()"/>
			</xsl:when>
		  </xsl:choose>
		</xsl:for-each>
