<xsl:stylesheet version="2.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns="http://www.w3.org/1999/xhtml" xmlns:msxsl="urn:schemas-microsoft-com:xslt" exclude-result-prefixes="msxsl">
  ...
  <!--	     This templae uses 'for-each-group' which requires xsl version 2.0 or higher                                                -->
  <!-- ================================================================================================================================ -->
  ...
  <xsl:param name="colWidth1">220</xsl:param>
    ...
    <xsl:if test="count($listRows) > 0">
      <xsl:for-each select="$listRows">
        <xsl:choose>
		  ...
		  <!-- ===================================================================================================================================================== -->
          <xsl:when test="$headerOnly = 1 and @Type = 'displaytext' and @Style = 'Panel' and not(.='') and not(name() = 'InspectionType' or (name() = 'ServiceDetails' and $timeValue = '') or (name() = 'Documents' and $imageCount = 0))">
			<p/>
            <table style="page-break-inside:avoid; table-layout:fixed; margin-left:{$tableIndent}px; width:{$tableWidth}px; font-family:'Calibri'; border-collapse: collapse;">
              ...
              <!-- ================================================================================================================================ -->
              <!-- Main section header - a taller row with background, followed by a narrow blank row                                               -->
              <!-- ================================================================================================================================ -->
              ...
              <!-- ================================================================================================================================ -->
              <!-- Footer - final blank row with bottom border                                                                                      -->
              <!-- ================================================================================================================================ -->
              <tr bgcolor="white" height="10px" style="page-break-inside:avoid;">
          ...
          <xsl:when test="$headerOnly = 0 and @Type='gmap' and @Description='Location'"></xsl:when>
          <xsl:when test="$headerOnly = 0 and @Type='gmap' and @Description='Map'"></xsl:when>
          <xsl:when test="$headerOnly = 0 and name()='ReadOnlyAuthorisationLabel'"></xsl:when>
          <xsl:when test="$headerOnly = 0 and name()='Location_Annotations'"></xsl:when>
          <xsl:when test="$headerOnly = 0 and name()='Map_Annotations'"></xsl:when>
          <xsl:when test="$headerOnly = 0 and name()='ConditionRptr'"></xsl:when>
          <xsl:when test="$headerOnly = 0 and name()='ObservationType'"></xsl:when>
          <xsl:when test="$headerOnly = 0 and name()='MeasuringDevice'"></xsl:when>
          <xsl:when test="$headerOnly = 0 and name()='TelemetryMeasuringDevice'"></xsl:when>                   <!-- Handled in Verified Meter section below -->
          <xsl:when test="$headerOnly = 0 and name()='TelemetryMeasuringDeviceSerial'"></xsl:when>             <!-- Handled in Verified Meter section below -->
          <xsl:when test="$headerOnly = 0 and name()='TelemetryDataMeasuringDevice'"></xsl:when>               <!-- Handled in Verified Meter section below -->
          <xsl:when test="$headerOnly = 0 and name()='TelemetryDataMeasuringDeviceSerial'"></xsl:when>         <!-- Handled in Verified Meter section below -->
          <!-- ================================================================================================================================ -->
          <xsl:when test="$headerOnly = 0 and name()='VerifiedMeter'">
 	        <xsl:variable name="outcome">
		      <xsl:call-template name="value"/>
	        </xsl:variable>
            <tr height="24px" style="page-break-inside:avoid; vertical-align:center; border-left:solid 1px {$backColor}; border-right:solid 1px {$backColor};">
              <td style="page-break-inside:avoid; text-indent: 20px; font-size: 10pt;">
                <xsl:call-template name="label" />
              </td>
              <td style="page-break-inside:avoid; font-size: 10pt;">
			    <xsl:choose>
				  <xsl:when test="$outcome = 'AS ABOVE'">
				    <xsl:choose>
					  <xsl:when test="contains($inspectionDescription,'Data')">
				        <xsl:value-of select="//FormData/TelemetryDataMeasuringDevice"/> SN: <xsl:value-of select="//FormData/TelemetryDataMeasuringDeviceSerial"/>
					  </xsl:when>
					  <xsl:otherwise>
				        <xsl:value-of select="//FormData/TelemetryMeasuringDevice"/> SN: <xsl:value-of select="//FormData/TelemetryMeasuringDeviceSerial"/>
					  </xsl:otherwise>
				    </xsl:choose>
				  </xsl:when>
				  <xsl:otherwise>
					<xsl:choose>
					  <xsl:when test="contains($inspectionDescription,'Data')">
				        <xsl:value-of select="//Interaction/Checklist/TelemetryDataMeasuringDevice"/> SN: <xsl:value-of select="//Interaction/Checklist/TelemetryDataMeasuringDeviceSerial"/>
					  </xsl:when>
					  <xsl:otherwise>
				        <xsl:value-of select="//Interaction/Checklist/TelemetryMeasuringDevice"/> SN: <xsl:value-of select="//Interaction/Checklist/TelemetryMeasuringDeviceSerial"/>
					  </xsl:otherwise>
					</xsl:choose>
				  </xsl:otherwise>
			    </xsl:choose>
          ...
          <!-- ================================================================================================================================ -->
          <xsl:when test="$headerOnly = 0 and @Type='displaytext' and not(.='') and not(@Description='Stuff')">
            <tr style="page-break-inside:avoid;">
              <td colspan="2" style="page-break-inside:avoid;">
                <br />
  ...
  <!-- ================================================================================================================================ -->
  <!-- Called from above to handle radio button values                                                                                  -->
  <!-- ================================================================================================================================ -->
